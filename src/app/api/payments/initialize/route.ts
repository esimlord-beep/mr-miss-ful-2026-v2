...
export async function POST(request: Request) {
  ...
  // Check if already processed — if webhook already handled it, skip
  ...
  try {
    ...
    // If payment record exists but not processed, process it
    // If no record exists, create and process it
    ...
    if (!payment) {
      ...
      if (paymentError) {
        // If insert failed due to duplicate, webhook already created it
        const { data: racePayment } = await adminSupabase
          .from("payments")
          .select("id, processed")
          .eq("transaction_reference", reference)
          .maybeSingle();
        
        if (racePayment?.processed) {
          return NextResponse.json({ processed: true, message: "Already processed by webhook." });
        }
        throw paymentError;
      }
    ...
    if (rpcError) {
      // If RPC failed, check if webhook already processed it
